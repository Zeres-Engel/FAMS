...
def load_arcface_model(model_name, checkpoint_path, device):
    if model_name == 'ir_18':
        model = arcface_network.iresnet18()
    elif model_name == 'ir_34':
        model = arcface_network.iresnet34()
...
def main():
    ...
    logger.info(f"Model type: {args.model_type}, Checkpoint path: {args.checkpoint_path}, Model name: {args.model_name}, Validation data path: {args.valid_path}")
    ...
    aka_auc = eval_metric.run(backbone=model, graph_name='m')

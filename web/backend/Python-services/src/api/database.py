...
async def import_users(
    ...
    This will automatically determine the grade based on current date and batch year
    and import data without generating schedules.
    """
    ...
    if not os.path.exists(file_path):
        ...
        if os.path.exists(alt_path):
            ...
            return ErrorResponseModel(
                "File not found",
                404,
                f"No Excel file found for grade {grade}. Please ensure the file exists at {file_path} or {alt_path}."
            )
    
    try:
        # Process the existing file without scheduling
        background_tasks.add_task(init_users_only_from_excel, [file_path])
        
        # Add a task to delete the file after processing
        background_tasks.add_task(lambda p=file_path: os.remove(p) if os.path.exists(p) else None)
        ...
        return ResponseModel(
            ...
            f"Users import started for batch {batchYear}-{batchYear+3} (Grade {grade})"
        )
    except Exception as e:
        return ErrorResponseModel(
            "Import failed",

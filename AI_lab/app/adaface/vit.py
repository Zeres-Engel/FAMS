...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.feature(x)
        norm = torch.norm(x, p=2, dim=1, keepdim=True)  # Tính chuẩn L2 của output
        output = torch.div(x, norm)  # Chuẩn hóa output bằng torch.div
        return output, norm  # Trả về cả output và norm